...
const ChatMessages = ({ receiver }: { receiver: userWithActivities }) => {
  ...
  const { fetchMessages, messages, sendMessage, setMessage, newMessageIds } =
    useChatStore();
...
`}
                    >
                      <div
                        className={`
    relative px-4 py-2 rounded-2xl text-sm leading-relaxed break-words transition-all duration-300
    ${
      isCurrentUser
      ...
      <div className="p-4 border-t border-zinc-700 bg-zinc-800/30 backdrop-blur-sm">
        <div className="flex gap-3 items-end">
          <div className="flex-1 relative">
            <Input
              placeholder={`Message ${
                receiver.firstName || receiver.username || "user"
              }...`}
              value={newMessage}
